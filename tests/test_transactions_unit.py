...
def _register_and_login(client, email="tx@example.com", password="password123"):
    reg = client.post(
        "/auth/register",
        json={"full_name": "TX User", "email": email, "password": password},
    )
...
def _ensure_categories(session, client, headers):
    """Make sure the user has spending categories assigned and return category name."""
    ...
    if user and user.financial_profile:
        user.financial_profile.spending_categories = ["Food & Dining", "Shopping"]
        session.commit()
    resp = client.get("/transactions/categories", headers=headers)
    if resp.status_code == 200 and resp.json():
        return next((c["name"] for c in resp.json() if c["is_default"]), resp.json()[0]["name"])
    return "Food & Dining"
...
def test_create_transaction_unauthorized(monkeypatch):
    ...
    try:
        response = client.post(
            "/transactions",
            json={"category": "Food", "amount": -50, "date": "2026-06-15"},
        )
...
def test_create_transaction_invalid_amount(monkeypatch):
    ...
    try:
        ...
        response = client.post(
            "/transactions",
            headers=headers,
            json={"category": cat_id, "amount": "not-a-number", "date": "2026-06-15"},
        )
...
def test_create_transaction_missing_required_fields(monkeypatch):
    ...
    try:
        headers = _register_and_login(client)
        response = client.post("/transactions", headers=headers, json={"amount": -50})
        assert response.status_code == 422
    finally:
        _cleanup()
        session.close()
...
def test_list_transactions_success(monkeypatch):
    ...
    try:
        ...
        client.post(
            "/transactions",
            headers=headers,
            json={"category": cat_id, "amount": -25.00, "date": "2026-06-10"},
        )
...
def test_list_transactions_pagination(monkeypatch):
    ...
    try:
        ...
        resp_page1 = client.get("/transactions?page=1&page_size=2", headers=headers)
        assert resp_page1.status_code == 200
        assert len(resp_page1.json()["items"]) == 2

        resp_page2 = client.get("/transactions?page=2&page_size=2", headers=headers)
...
def test_update_transaction_success(monkeypatch):
    ...
    try:
        ...
        created = client.post(
            "/transactions",
            headers=headers,
            json={"category": cat_id, "amount": -30.00, "date": "2026-06-10"},
        )
...
def test_delete_transaction_success(monkeypatch):
    ...
    try:
        ...
        assert all(item["id"] != tx_id for item in list_resp.json()["items"])
    finally:
        _cleanup()
        session.close()
...
def test_get_transaction_by_id(monkeypatch):
    ...
    try:
        ...
        found = next((item for item in list_resp.json()["items"] if item["id"] == tx_id), None)
